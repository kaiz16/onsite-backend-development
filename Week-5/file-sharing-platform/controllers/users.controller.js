...
async function deleteUser(req, res) {
  try {
    ...
    // Delete user by id.
    // NOTE: Deleting a user will also delete all files associated with the user.
...
async function getUserFiles(req, res) {
  try {
    // Find all files created by the user.
    const files = await File.findAll({
      where: {
        userId: parseInt(req.params.id),
        ...(parseInt(req.params.id) !== req.user.id && { isPublic: true }),
      },
    });
